...
const TreemapTooltip = ({ data, totalValue, hierarchy, date, colors }) => {
    ...
    useEffect(() => {
        console.log('hierarcy is now', hierarchy.data.children);
    }, [data]);
    ...
    function getClosePrice(date, stock, period) {
        ...
        if (!currentData || !pastData) {
            console.log(`Missing price data for ${stock} on required dates.`);
            return null;
        }
    ...
    return (
        <div className={styles.tooltipContainer}>
            ...
            <div style={{ marginTop: '.5rem' }}>
                ...
                <div style={{ margin: '.5rem auto 0 auto', backgroundColor: '#CFE0EF', fontWeight: 'bold', width: 'fit-content', padding: '.5rem', borderRadius: '4px' }}>Weight: {`${((data.value / totalValue) * 100).toFixed(2)}%`}</div>
            </div>
